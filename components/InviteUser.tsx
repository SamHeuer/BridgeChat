  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    if (!isPro && noOfUsersInChat >= 2) {
      toast({
        ...
        description:
          "You have exceeded the limit of users in FREE plan. Upgrade to PRO to add more friends in this chat!",
        variant: "destructive",
        action: (
          <ToastAction
            altText="Upgrade"
    ...
    if (querySnapshot.empty) {
      toast({
        ...
        description:
          "Please enter a valid email address ! OR resend the invitation once they have signed up!",
        variant: "destructive",
      });
      return;
    } else {
      ...
      await setDoc(addChatRef(chatId, user.id), {
        ...
        .catch(() => {
          toast({
            title: "Error",
            description:
              "Whoops! Something went wrong while adding the user to the chat! ❌",
            variant: "destructive",
          });
          setOpen(false);
        });
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Input
                          placeholder="language@nobarrier.com"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
